...
export function PwaInstallPrompt() {
  ...
  const copy = language === 'ar' ? { label: 'تثبيت التطبيق', title: 'ثبّت Sessions Archive', hint: iosHint ? 'على iPhone أو iPad: افتح المشاركة ثم اختر «إضافة إلى الشاشة الرئيسية».' : 'افتح المنصة كتطبيق مستقل من الشاشة الرئيسية.', install: 'تثبيت التطبيق', dismiss: 'إخفاء اقتراح التثبيت' } : { label: 'Install app', title: 'Install Sessions Archive', hint: iosHint ? 'On iPhone or iPad: open Share, then choose “Add to Home Screen”.' : 'Open the platform as a standalone app from your home screen.', install: 'Install app', dismiss: 'Dismiss install suggestion' }
  return <aside className="pwa-install" aria-label={copy.label}><img src={`${import.meta.env.BASE_URL}icon-192.png?v=4`} alt="" aria-hidden="true" /><div><strong>{copy.title}</strong><span>{copy.hint}</span></div>{installPrompt && <button className="button button-primary" type="button" onClick={() => void install()}>{copy.install}</button>}<button className="pwa-dismiss" type="button" onClick={dismiss} aria-label={copy.dismiss}>×</button></aside>
}
