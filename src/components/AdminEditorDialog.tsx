      ...
      switch (activeTarget.type) {
        ...
        case 'session': {
          ...
          await onSave(activeTarget, {
            ...
            starts_at: startsAt ? new Date(startsAt).toISOString() : activeTarget.item.starts_at,
            ...
            series_id: seriesId,
            series_position: seriesId ? Math.max(1, Number(form.get('series_position') || 1)) : null,
            status: String(form.get('status') || 'published'),
            is_pinned: form.get('is_pinned') === 'on',
            cover_focus_x: Math.min(100, Math.max(0, Number(form.get('cover_focus_x') || 50))),
            cover_focus_y: Math.min(100, Math.max(0, Number(form.get('cover_focus_y') || 50))),
          })
          break
        }
        case 'video': {
          ...
          if (!source) throw new Error(ar ? 'أدخل رابطًا صالحًا من YouTube أو Google Drive أو WhatsApp أو Telegram.' : 'Enter a valid YouTube, Google Drive, WhatsApp, or Telegram URL.')
          await onSave(activeTarget, {
            title: String(form.get('title') || '').trim(),
  ...
  return <div className="editor-backdrop" role="presentation" onMouseDown={(event) => { if (event.currentTarget === event.target && !busy) onClose() }}>
    <section className="admin-editor" role="dialog" aria-modal="true" aria-labelledby="admin-editor-title">
      ...
      <form onSubmit={(event) => void submit(event)}><div className="editor-body">
        ...
        {activeTarget.type === 'session' && <>
          <section className="editor-section"><h3>{ar ? 'المحتوى الأساسي' : 'Core content'}</h3><div className="editor-grid"><Field label={ar ? 'عنوان السيشن' : 'Session title'}><input name="title" defaultValue={activeTarget.item.title} required /></Field><Field label="Slug"><input name="slug" defaultValue={activeTarget.item.slug} required /></Field><Field label={ar ? 'الوصف' : 'Description'} wide><textarea name="description" defaultValue={activeTarget.item.description} rows={6} required /></Field></div></section>
          <section className="editor-section"><h3>{ar ? 'موضع الغلاف في الكارد' : 'Card cover focus'}</h3><CoverFocusEditor key={activeTarget.item.id} session={activeTarget.item} language={language} /></section>
          <section className="editor-section"><h3>{ar ? 'الموعد والمكان' : 'Schedule and place'}</h3><div className="editor-grid"><Field label={ar ? 'وقت البداية' : 'Start time'}><input name="starts_at" type="datetime-local" defaultValue={dateTimeLocal(activeTarget.item.starts_at)} required /></Field><Field label={ar ? 'وقت النهاية' : 'End time'}><input name="ends_at" type="datetime-local" defaultValue={dateTimeLocal(activeTarget.item.ends_at)} /></Field><Field label={ar ? 'المكان / رابط الحضور' : 'Location / meeting link'}><input name="location" defaultValue={activeTarget.item.location ?? ''} /></Field><Field label={ar ? 'السعة' : 'Capacity'}><input name="capacity" type="number" min="1" defaultValue={activeTarget.item.capacity} required /></Field></div></section>
          <section className="editor-section"><h3>{ar ? 'التصنيف والتنظيم' : 'Classification and structure'}</h3><div className="editor-grid"><Field label={ar ? 'التصنيف' : 'Category'}><select name="category_id" defaultValue={activeTarget.item.category_id ?? ''}><option value="">{ar ? 'بدون تصنيف' : 'No category'}</option>{categories.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></Field><div className="form-field wide"><span className="field-label">{ar ? 'المتحدثون' : 'Speakers'}</span><div className="speaker-multi-select" role="group" aria-label={ar ? 'اختيار متحدثي السيشن' : 'Choose session speakers'}>{speakers.map(item => <label className="speaker-choice" key={item.id}><input type="checkbox" name="speaker_ids" value={item.id} defaultChecked={selectedSpeakerIds.has(item.id)} /><span><strong>{item.name}</strong>{item.organization && <small>{item.organization}</small>}</span></label>)}</div><span className="field-hint">{ar ? 'اختر متحدثًا واحدًا أو أكثر. أول اسم محدد يُحفظ أيضًا كمتحدث أساسي للتوافق.' : 'Choose one or more speakers. The first selected speaker is also kept as the primary speaker for compatibility.'}</span></div><Field label={ar ? 'السلسلة' : 'Series'}><select name="series_id" defaultValue={activeTarget.item.series_id ?? ''}><option value="">{ar ? 'بدون سلسلة' : 'No series'}</option>{series.map(item => <option key={item.id} value={item.id}>{item.title}</option>)}</select></Field><Field label={ar ? 'رقم الجزء' : 'Part number'}><input name="series_position" type="number" min="1" defaultValue={activeTarget.item.series_position ?? 1} /></Field><Field label={ar ? 'حالة السيشن' : 'Session status'}><select name="status" defaultValue={activeTarget.item.status}><option value="draft">{ar ? 'مسودة' : 'Draft'}</option><option value="published">{ar ? 'منشور' : 'Published'}</option><option value="cancelled">{ar ? 'ملغي' : 'Cancelled'}</option></select></Field><label className="pin-session-control wide"><input type="checkbox" name="is_pinned" defaultChecked={Boolean(activeTarget.item.is_pinned)} /><span><strong>{ar ? 'تثبيت السيشن في أول الصفحة' : 'Pin session to the top'}</strong><small>{ar ? 'عند تثبيت هذه السيشن سيتم فك تثبيت أي سيشن أخرى تلقائيًا.' : 'Pinning this session automatically unpins the previously pinned session.'}</small></span></label></div></section>
        </>}
        {activeTarget.type === 'video' && <section className="editor-section"><h3>{ar ? 'بيانات التسجيل' : 'Recording details'}</h3><div className="editor-grid"><Field label={ar ? 'عنوان التسجيل' : 'Recording title'}><input name="title" defaultValue={activeTarget.item.title} required /></Field><Field label={ar ? 'السيشن' : 'Session'}><select name="session_id" defaultValue={activeTarget.item.session_id} required>{sessions.map(item => <option key={item.id} value={item.id}>{item.title}</option>)}</select></Field><Field label="Part" hint={ar ? 'اختر رقم الجزء الذي سيظهر فيه هذا التسجيل.' : 'Choose which part should contain this recording.'}><input name="part_number" type="number" min="1" step="1" defaultValue={activeTarget.item.part_number ?? 1} required /></Field><Field label={ar ? 'الترتيب داخل الـPart' : 'Position in part'}><input name="position" type="number" min="0" step="1" defaultValue={activeTarget.item.position} /></Field><Field label={ar ? 'رابط التسجيل' : 'Recording URL'} hint={ar ? 'يقبل YouTube أو Google Drive أو رابط رسالة WhatsApp أو Telegram. روابط WhatsApp وTelegram ستظهر كبطاقة تفتح الرسالة الأصلية.' : 'Accepts YouTube, Google Drive, WhatsApp message, or Telegram message links. WhatsApp and Telegram open as external message cards.'} wide><input name="video_url" inputMode="url" defaultValue={videoSourceUrl(activeTarget.item.video_provider, activeTarget.item.youtube_video_id)} required /></Field></div></section>}
